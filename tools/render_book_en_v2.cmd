...
rem Render EN book v2 Markdown to HTML and PDF (self-contained).
rem Expects docs\book\en_v2\_compiled_v2*.md to already exist.
rem Usage:
rem   tools\render_book_en_v2.cmd
rem   tools\render_book_en_v2.cmd draft
rem   tools\render_book_en_v2.cmd print
rem   tools\render_book_en_v2.cmd print quick
...
set IN=docs\book\en_v2\_compiled_v2.md
if /I "%MODE%"=="print" set IN=docs\book\en_v2\_compiled_v2_print.md
if /I "%VARIANT%"=="quick" set IN=docs\book\en_v2\_compiled_v2_quick.md
if /I "%MODE%"=="print" if /I "%VARIANT%"=="quick" set IN=docs\book\en_v2\_compiled_v2_quick_print.md
...
set "BOOK_TITLE=DETM - EN v2"
if /I "%VARIANT%"=="quick" set "BOOK_TITLE=DETM - EN v2 (quick)"
if /I "%MODE%"=="draft" set "BOOK_TITLE=%BOOK_TITLE% (draft)"
"%PANDOC%" "%IN%" --from gfm --to html5 --standalone --embed-resources --metadata title="%BOOK_TITLE%" --metadata lang=en --resource-path="docs/book/en_v2;docs/book/en_v2/assets;." --output "%OUT_HTML%"
...
:try_browser
set "CHROME_PROFILE=docs\\book\\en_v2\\_tmp\\chrome_profile_%MODE%_%VARIANT%"
if not exist "docs\\book\\en_v2\\_tmp" mkdir "docs\\book\\en_v2\\_tmp" >nul 2>nul
if exist "%CHROME_PROFILE%" rmdir /s /q "%CHROME_PROFILE%" >nul 2>nul
if exist "%CHROME_PROFILE%" (
  set "CHROME_PROFILE=docs\\book\\en_v2\\_tmp\\chrome_headless_%RANDOM%"
)
...
if errorlevel 1 (
  ...
  echo Tip: install Google Chrome or Microsoft Edge, or install rsvg-convert and re-run.
  popd
  exit /b 1
)
...
echo Tip: install a LaTeX distribution (TeX Live / MiKTeX), or install Chrome/Edge for browser export.
popd
exit /b 1

:have_xelatex
if exist "%OUT_PDF_TMP%" del /f /q "%OUT_PDF_TMP%" >nul 2>nul
"%PANDOC%" "%IN%" --from gfm --resource-path="docs/book/en_v2;docs/book/en_v2/assets;." --pdf-engine=xelatex --output "%OUT_PDF_TMP%"
