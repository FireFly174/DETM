...
### Нормировки (важно для сравнения по N)

При изменении размера решётки `N` сравнение корректно только при явной нормировке начальных условий и/или суммарной энергии. Типичные варианты:
- фиксировать `E_total = const` (при росте `N` «размазывает» энергию по узлам);
- фиксировать `mean(E) = const` (интенсивная нормировка; сравнима «по плотности»);
- фиксировать `E_total ∝ N` или `E_total ∝ N^2` (чтобы удерживать специфический масштаб под выбранную интерпретацию).

Выбор нормировки должен быть указан в `meta/config` прогона и учитываться при сравнении метрик.
