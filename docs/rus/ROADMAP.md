...
- `readout` (2026-03-20): в `detm.runtime.pattern_memory` добавлены first-class `BridgeRecordSource` и `VerificationRun` contracts с local file-backed stores. Observe-only multiscale runtime теперь поднимает durable source records для зрелых кандидатов, связывает `BridgeRecord.db_refs.source_id` с cold-source сущностью и пишет как source-level verification summary (`verification_count/success_count/failure_count/last_status`), так и отдельные durable verification-run records. App-layer writer пишет derived artifacts `bridge_record_sources.jsonl` и `bridge_verifications.jsonl`, а analytics ingest индексирует их как run-level files + logical-row refs и дополнительно нормализует verification layer в first-class read-model: таблица `bridge_verification_runs`, export `analytics/verification_windows.jsonl`, `run_summary.counts.verification_run_count` и `run_summary.final_verification_run`. Это всё ещё не trajectory store DB и не hot Redis transport semantics: full forward/reverse bodies, verification orchestration beyond observe-only reuse, и live `hint/jump` по-прежнему остаются незакрытой частью `MSC-02`.
