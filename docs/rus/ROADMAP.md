...
- `readout` (2026-03-20): в `detm.runtime.pattern_memory` добавлен first-class `BridgeRecordSource` contract и local file-backed source store; observe-only multiscale runtime теперь поднимает durable source records для зрелых кандидатов, связывает `BridgeRecord.db_refs.source_id` с cold-source сущностью и пишет observe-only verification summary (`verification_count/success_count/failure_count/last_status`) при повторных наблюдениях source. App-layer writer пишет новые derived artifacts `bridge_record_sources.jsonl` и `bridge_verifications.jsonl`, а analytics ingest индексирует их как run-level files + logical-row refs. Это всё ещё не trajectory store DB и не hot Redis transport semantics: full forward/reverse bodies, отдельная durable `VerificationRun` model и live `hint/jump` по-прежнему остаются незакрытой частью `MSC-02`.
