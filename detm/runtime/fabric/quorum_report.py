...
def _normalize_replay_policy_tier(raw: str | object) -> str:
...
class FabricQuorumReportBuilder:
    ...
    def build(
        ...
        *,
        ...
        configured_mode = _normalize_delivery_guarantee_mode(self.delivery_guarantee_mode)
        at_least_once_enabled = bool(int(self.delivery_required_receipts) > 0)
        idempotent_ingress_enabled = bool(self.transport_dedup_ingress_enabled)
        if at_least_once_enabled and idempotent_ingress_enabled:
            effective_mode = "at_least_once_idempotent"
        elif at_least_once_enabled:
            effective_mode = "at_least_once"
        else:
            effective_mode = "best_effort"
        quorum_report["delivery_guarantees"] = {
            "configured_mode": configured_mode,
            "effective_mode": effective_mode,
            "at_least_once_enabled": at_least_once_enabled,
            "idempotent_ingress_enabled": idempotent_ingress_enabled,
            "degraded": bool(configured_mode != effective_mode),
        }
        ...
        replay_tier = _normalize_replay_policy_tier(self.replay_policy_tier)
        replay_enabled = bool(
            replay_tier == "strict_window" or (replay_tier == "sampled" and int(replay_sample_stride) > 0)
        )
        quorum_report["replay_sampling"] = {
            "enabled": replay_enabled,
            "tier": replay_tier,
            "sample_stride": int(replay_sample_stride),
            "strict_window_size": max(1, int(self.replay_strict_window_size)),
            "checks_total": int(replay_checks_total),
            "checks_failed": int(replay_checks_failed),
        }
        validator_registry = quorum_report.get("validator_registry")
        validators = []
        if isinstance(validator_registry, dict):
            validators = [str(v) for v in list(validator_registry.get("validators", []))]
        membership_hardening = quorum_report.get("membership_hardening", {})
        profile = _normalize_handshake_profile(self.handshake_profile)
        hardening_map = membership_hardening if isinstance(membership_hardening, dict) else {}
        quorum_report["handshake_profile"] = {
            "profile": profile,
            "strict_runtime_enforcement": bool(profile == "production"),
            "active_hardening": {
                "enforce_required_validator_ids": bool(quorum_report.get("enforce_required_validator_ids", False)),
                "enforce_active_validator_membership": bool(
                    hardening_map.get("enforce_active_validator_membership", False)
                ),
                "enforce_ack_sender_validator_match": bool(
                    hardening_map.get("enforce_ack_sender_validator_match", False)
                ),
                "enforce_ack_auth_key_id_binding": bool(
                    hardening_map.get("enforce_ack_auth_key_id_binding", False)
                ),
                "enforce_ack_transport_identity_binding": bool(
                    hardening_map.get("enforce_ack_transport_identity_binding", False)
                ),
            },
        }
        quorum_report["validator_coordination"] = {
            "validator_count": int(len(validators)),
            "validator_ids": sorted(validators),
            "membership_hardening": hardening_map,
        }
