...
from detm.runtime.fabric_delivery import OUTBOX_DROP_POLICIES, JsonlFabricEnvelopeOutbox
...
from detm.runtime.fabric_tcp_transport import open_fabric_transport
from detm.runtime.fabric_transport import BACKPRESSURE_POLICIES, BufferedFabricTransport, FabricTransportAdapter
...
def compose_fabric_handshake_runtime(
    *,
    ...
    replay_enabled = int(rec.replay_sample_stride) > 0
    validator = LocalFabricValidator(
        validator_id=f"validator.{rec.node_id}",
        replay_policy=ReplaySamplePolicy(
            enabled=bool(replay_enabled),
            sample_stride=max(1, int(rec.replay_sample_stride or 1)),
            sample_offset=0,
        ),
