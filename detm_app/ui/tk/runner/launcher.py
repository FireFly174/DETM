...
def launch_tk_ui(settings: UiRunSettings) -> None:  # pragma: no cover
    ...
    batch_running = False
    batch_handle: BatchRunHandle | None = None
    ...
    def _poll_batch_queue() -> None:
        nonlocal batch_running, batch_handle
        if batch_handle is None:
            return
        items, done = batch_handle.drain_messages()
        for item in items:
            _log(str(item))
        if done:
            batch_running = False
    ...
    def on_batch_toggle() -> None:
        ...
        _log(build_batch_start_message(request))

        batch_running = True
        batch_handle = start_batch_run(request)
        root.after(75, _poll_batch_queue)
